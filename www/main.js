...
jQuery(($) => {
	...
	$('.gear-icon').each(function() {
		$(this).data('id', 0).append($make('img').attr('src', TRANSPARENT))
	})
	...
	ipc.on('character', (character) => {
		...
		$.each(prefilter, (type, items) => {
			prefilter[type] = items.map(id => itemData.items[id]).filter((item) => {
	...
	ipc.on('outfit', function(outfit, override) {
		...
		SLOTS.forEach(function(slot) {
			...
			$('.equipped', elem).data('id', id).children('img').attr('src', img)
			// remodel
			...
			if (typeof over !== 'undefined') {
				...
				$('.override', elem).data('id', id).children('img').attr('src', img)
		...
		$('.typeahead').typeahead('destroy').off('.aa').val('').typeahead({
			...
			templates: {
				suggestion: function(item) {
					...
					var suggestion = $make('div').append($make('div', 'icon').append($make('img').attr('src', `img/${item.icon}.png`)), $make('div', 'info').append($make('span', 'name').text(item.name), $extra, $make('br'), $make('span', 'desc').html(item.tooltip)))
			...
			$override.data('id', choice.id).children('img').attr('src', `img/${choice.icon}.png`)
