...
jQuery(($) => {
  ...
  const SLOTS = [
    'weapon', 'chest', 'gloves', 'boots', 'innerwear',
    'hairAdornment', 'mask', 'costume', 'back', 'weaponSkin',
  ];
  ...
  ipc.on('character', (character) => {
    ...
    prefilter = {
      ...
      gloves: armor.gloves,
      boots: armor.boots,
      innerwear: gear.underwear,

      hairAdornment: [].concat(style.hair, gear.hair),
      mask: [].concat(style.face, gear.face),
      costume: style.body,
      back: style.back,
      weaponSkin: style.weapon[jobName],
    };
  ...
  ipc.on('outfit', function(outfit, override) {
    ...
    SLOTS.forEach(function(slot) {
      ...
      var img = (item ? `img/${item.icon}.png` : TRANSPARENT);
      ...
      if (typeof over !== 'undefined') {
        ...
        img = (item.id ? `img/${item.icon}.png` : TRANSPARENT);
  ...
  $('.toggle-display').click(function(e) {
    ...
    var item = getItem(slot, id);
    $equipped.find('img').attr('src', item ? `img/${item.icon}.png` : TRANSPARENT);
